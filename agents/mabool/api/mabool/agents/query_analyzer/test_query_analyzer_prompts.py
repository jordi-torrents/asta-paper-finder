...
def test_identify_relevance_criteria_output_valid() -> None:
    ...
    output = IdentifyRelevanceCriteriaOutput(required_relevance_criteria=criteria)
    assert output.required_relevance_criteria == criteria
...
def test_identify_relevance_criteria_output_duplicate_criterion_names() -> None:
    ...
    with pytest.raises(ValidationError):
        IdentifyRelevanceCriteriaOutput(required_relevance_criteria=criteria)

    with pytest.raises(ValidationError):
        IdentifyRelevanceCriteriaOutput(
            required_relevance_criteria=criteria[:1],
...
def test_identify_relevance_criteria_output_with_clarification_questions() -> None:
    ...
    assert output.required_relevance_criteria is None
    assert output.nice_to_have_relevance_criteria is None
...
def test_identify_relevance_criteria_output_with_nice_to_have_criteria() -> None:
    ...
    output = IdentifyRelevanceCriteriaOutput(
        required_relevance_criteria=required_criteria,
        nice_to_have_relevance_criteria=nice_to_have_criteria,
    )
    assert output.required_relevance_criteria == required_criteria
